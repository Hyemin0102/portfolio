...
### 💻contact 채팅창 구현
조금 더 인터렉션한 웹페이지를 구현하고자 contact 부분을 채팅창 형식으로 만들어 모달 대화 상자를 만들었습니다. 채팅 화면을 스크롤 하기 위해 useRef로 모달 내부의 스크롤 요소를 참조하고, useEffect로 컴포넌트가 렌더링 될 때마다 useRef로 참조한 요소의 가장 아래로 스크롤을 조정했습니다. (scrollTop은 스크롤바의 상단 가장자리와 요소의 상단 가장자리 사이의 거리인데 이 거리를 요소의 높이로 맞춤으로 스크롤 가장 아래로 유지)
```javascript
